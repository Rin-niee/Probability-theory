...
n1=(k+q)/p #20 = 0.8*n - 0,2 => (k+q)/p
n1=round(n1, 0)
n2 = (k-p)/p #20 = 0.8*n + 0.8 => (k-p)/p
n2= round(n2, 0)
print("надо произвести", n2, " - ",n1, " бросков")
...
print("наиболее вероятное число опоздавших", k2, "-", k1)
...
n = int(input("Введите число изделий"))
p = float(input("Введите процент повреждаемых при транспортировке изделий"))/100
L = n*p
k1 = int(input("Введите число повреждаемых при транспортировке изделий"))
PnA1 = ((E**(-L))*(L**k1))/(factor(k1))
print(PnA1)
o=0
sum = 0
while o<k1:
    sum+= ((E**(-L))*(L**o))/(factor(o))
    o = o+1
print(1-sum)
print(PnA1)
PnA2= 1 - ((E**(-L))*(L**k1))/(factor(k1))
print(PnA2)
...
print("В страховой компании 10 тыс. клиентов. Страховой взнос каждого клиента составляет 500 руб. При наступлении страхового случая, вероятность которого по имеющимся данным и оценкам экспертов можно считать равной p = 0,005, страховая компания обязана выплатить клиенту страховую сумму размером 50 тыс. руб. На какую прибыль может рассчитывать страховая компания с надежностью 0,95?")
n = int(input("Введите число клиентов")) #10000
n1 = int(input("Введите cтраховой взнос каждого клиента")) #500
n2 = int(input("Введите выплату клиенту страховой суммы")) #50000
p = float(input("Введите вероятность наступления страхового случая"))
PP = float(input("Введите надежность "))
q = 1-p
#P1(0<=k<=n0)= F(x2)- F(x1)
x1 = (0-n*p)/((n*p*q)**(1/2))
Fx1 = -0.5
